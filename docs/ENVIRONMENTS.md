...
Use the full setup script for the evaluation environment:
...
### Clean-checkout fixture contract

The Kakao transit observation used by the grounded grader is committed at
`backend/tests/fixtures/20260902-mapoderm-transit.json`. The casebook binds
the observation to the recorded request and response hashes. A clean checkout
therefore runs the focused grader tests without restoring a private local file.
