...
const ReviewEvidence = forwardRef<ReviewEvidenceHandle, {
  ...
  useImperativeHandle(ref, () => ({
    showReview(evidenceId) {
      ...
      if (index < 0) return;
      setExpanded(true);
      setPage(pages.findIndex(candidate => candidate.start <= index && index < candidate.end));
  ...
  for (let start = 0; start < visible.length;) {
    const remaining = visible.length - start;
    let size = Math.min(start === 0 ? FIRST_PAGE_SIZE : NEXT_PAGE_SIZE, remaining);
    if (remaining - size === 1) size += size < NEXT_PAGE_SIZE ? 1 : -1;
  ...
  const pageEnd = pages[currentPage]?.end ?? 0;
