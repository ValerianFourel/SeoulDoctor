...
async function main() {
  ...
  try {
    ...
    for (const viewport of [{ width: 1280, height: 800 }, { width: 390, height: 844 }]) {
      ...
      await page.route('**/*', async route => {
        ...
        return route.fulfill({
          ...
          body: JSON.stringify({
            ...
            results: [{ ...facility(korean ? 'Korean' : 'English'), answer_status: fallback ? 'fallback' : 'generated' }],
          }),
        });
      });
      ...
      await page.getByRole('button', { name: 'Review 1 from Synthetic clinic', exact: true }).first().click();
      await check(`${label} old message citation remains scoped`, longReview.evaluate(element => document.activeElement === element));
      await send('transport failure');
      await check(`${label} later HTTP failure preserves cards`, page.locator('section[data-facility-id="alpha"]').count().then(count => count === 3));
      const beforeRace = requestCount;
      await input.fill('slow request');
      await input.evaluate(element => {
        element.dispatchEvent(new KeyboardEvent('keydown', { key: 'Enter', bubbles: true }));
        element.dispatchEvent(new KeyboardEvent('keydown', { key: 'Enter', bubbles: true }));
        document.querySelector('button[aria-label="Share current location"]')?.click();
      });
