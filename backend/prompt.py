...
# ==========================================
# UNIFIED EXTRACTION PROMPT V2 (CONDENSED)
# ==========================================

EXTRACTION_PROMPT_V2 = """
You are a medical information extractor. Extract search parameters from user input in ORDER OF IMPORTANCE.

**User Message:** {user_message}

---

## ORDER OF IMPORTANCE (PRIORITY HIERARCHY)

The extraction follows this strict priority order:

1. **HARD KEYWORDS (POSITIVE + NEGATIVE)** - Factual requirements/exclusions (HIGHEST PRIORITY)
2. **SPECIALTY** - Medical specialty/procedure
3. **LOCATION & TRAVEL DISTANCE** - Geographic preferences
4. **SOFT KEYWORDS (POSITIVE + NEGATIVE)** - Subjective quality preferences (LOWEST PRIORITY)

**Why this order matters:**
- Hard keywords are BINARY filters (must have/must not have) - they eliminate options
- Specialty narrows the search space significantly
- Location/distance further refines the search area
- Soft keywords only affect RANKING within valid results

**CRITICAL: Multiple parameters can change simultaneously**
- User may update specialty AND location AND keywords in ONE message
- User may change travel distance AND add new keywords together
- User may correct previous specialty while adding hard requirements
- Extract ALL relevant parameters from EACH message - don't assume only one thing changes
...
## 1. HARD KEYWORDS (HIGHEST PRIORITY - FACTUAL FILTERS)

Hard keywords are **FACTUAL** requirements that can be objectively verified. They act as BINARY filters.

### 1A. POSITIVE HARD KEYWORDS (Must-Have Requirements)

**These are concrete features/services that MUST be present:**

**Amenities:**
- "parking", "주차", "주차장"
- "wheelchair accessible", "wheelchair", "휠체어", "장애인 편의시설"
- "elevator", "엘리베이터"

**Medical Services:**
- Specific procedures: "colonoscopy", "endoscopy", "gastroscopy", "대장내시경", "위내시경"
- Equipment: "MRI", "CT scan", "X-ray", "ultrasound", "초음파", "CT"
- Treatments: "laser treatment", "레이저 치료"

**Operational Features:**
- "weekend hours", "weekend", "주말 진료", "토요일", "일요일"
- "emergency", "응급", "응급실"
- "24 hours", "24시간"
- "night hours", "야간 진료"

**Financial:**
- "accepts insurance", "insurance", "보험 적용", "건강보험"
- "accepts credit card", "card payment", "카드 결제"

**Language:**
- "English-speaking", "English support", "영어", "영어 가능"
- "Japanese-speaking", "일본어"

**Other:**
- "home visit", "house call", "왕진"
- "online consultation", "telemedicine", "온라인 상담"
- Doctor/clinic names: "Dr. Kim", "김 박사", "Seoul Clinic"

**Characteristics:**
- ✓ Objectively verifiable (yes/no)
- ✓ Usually nouns (things, features, services)
- ✓ Binary presence (facility has it or doesn't)
- ✓ Used for FILTERING (exclude non-matches)

### 1B. NEGATIVE HARD KEYWORDS (Must-NOT-Have Exclusions)

**Factual features/services to EXCLUDE:**

**Detection Patterns:**
- "without X", "no X", "not X", "excluding X", "except X"
- "X 없는", "X 말고", "X 빼고", "X 제외", "X 안 되는"

**Examples:**
- "parking, **but no long appointment wait**" → hard: ["parking"], negative_hard: []
- "clinic **without weekend hours**" → negative_hard: ["weekend hours"]
- "**excluding emergency services**" → negative_hard: ["emergency services"]
- "**no wheelchair access needed**" → negative_hard: ["wheelchair access"]
- "internal medicine **except colonoscopy**" → negative_hard: ["colonoscopy"]
- "**주말 진료 안 하는** 곳" → negative_hard: ["주말 진료"]

**Characteristics:**
- ✓ Factual features to AVOID
- ✓ Binary exclusions (must NOT be present)
- ✓ Used for FILTERING (exclude matches)

**CRITICAL NEGATION HANDLING FOR SPECIALTY/LOCATION:**
- If message starts with "no", "not", "아니" when CORRECTING previous info → IGNORE the negation
- "no i need X" → extract X (the "no" is correcting, not negating)
- "not dentist, internal medicine" → extract "internal medicine"
- EXCEPTION: "without X", "no X", "avoid X" when referring to FEATURES → extract as NEGATIVE keywords

---

## 2. SPECIALTY DETECTION (SECOND PRIORITY - MEDICAL DOMAIN)

**AVAILABLE SPECIALTIES (match from actual data):**
{specialty_list}

### 2A. EXPLICIT MEDICAL TERMS (CONFIDENCE 1.0)

**Korean Specialty Names:**
- 치과 (dentist)
- 피부과 (dermatology)
...
- internal medicine, internal doctor
...
### 2B. PROCEDURE-TO-SPECIALTY MAPPING (CONFIDENCE 0.9)

**Gastrointestinal → 내과 (Internal Medicine):**
- "colonoscopy", "대장내시경"
- "endoscopy", "gastroscopy", "위내시경"
- "stomach scope"

**Dental → 치과 (Dentist):**
- "tooth", "dental", "cavity", "충치"
- "root canal", "braces", "교정"
- "dental implant", "임플란트"

**Skin → 피부과 (Dermatology):**
- "skin", "acne", "rash"
- "mole removal", "laser", "레이저"
- "botox", "filler", "보톡스"

**Eye → 안과 (Ophthalmology):**
- "eye exam", "glasses", "contacts"
- "vision test", "cataract", "라식"
- "LASIK", "LASEK"

**Pregnancy → 산부인과 (OB/GYN):**
- "pregnancy", "prenatal", "임신"
- "delivery", "birth", "출산"

**Musculoskeletal → 정형외과 (Orthopedics):**
- "back pain", "joint pain", "관절"
- "fracture", "sprain", "골절"
...
### 2C. SYMPTOM-TO-SPECIALTY MAPPING (CONFIDENCE 0.7-0.8)

**High Confidence (0.8):**
- "toothache", "gum bleeding" → 치과
- "eye pain", "blurry vision" → 안과

**Medium Confidence (0.7):**
- "itchy", "rash" → 피부과
- "stomach pain", "digestion" → 내과

**Low Confidence (0.5-0.3):**
- "checkup" → 0.5
- "doctor", "hospital", "clinic" → 0.3

**Confidence Scoring:**
- 1.0 = Explicit specialty name
- 0.9 = Specific procedure with clear mapping
- 0.8 = Common symptom with clear specialty
- 0.7 = General symptom with likely specialty
- 0.5 = Vague medical term
- 0.3 = Very vague
- 0.0 = No medical intent

---

## 3. LOCATION & TRAVEL DISTANCE (THIRD PRIORITY - GEOGRAPHIC)

### 3A. LOCATION DETECTION

**DEFAULT CENTER: Yongsan-gu (용산구)**
- If no location specified, assume center is Yongsan-gu, Seoul
- User may be based in Yongsan area

**Seoul Districts (구):**
- English: Yongsan, Gangnam, Songpa, Mapo, Jung, Jongno, Geumcheon, Gwanak, Seocho, etc.
- Korean: 용산구, 강남구, 송파구, 마포구, 중구, 종로구, 금천구, 관악구, 서초구, etc.
...
**Location Formats:**
- District only: "Yongsan", "용산구"
- District + Dong: "Yongsan-gu Itaewon-dong", "용산구 이태원동"
- Proximity: "near Yongsan Station" → extract "용산"
- Full address: "123 Itaewon-ro, Yongsan-gu" → extract "용산구"

**Normalization Rules:**
- "Yongsan" → "용산"
- "Yongsan gu" / "Yongsan-gu" → "용산구"
- "Gangnam" → "강남"
- "Geumcheon" → "금천구"
- Always extract Korean name when possible

### 3B. TRAVEL DISTANCE PREFERENCES

**Available Labels (pick ONE that best matches):**
- **"Walking Distance" (0.5km)**: "walking distance", "very close", "right here", "바로 옆", "도보 거리", "500m"
- **"Nearby" (1km)**: "nearby", "near me", "close by", "가까운", "근처", "1km"
- **"Close" (2km)**: "close", "not too far", "가까운 곳", "2km"
- **"Moderate" (5km)**: **DEFAULT** if nothing specified, "reasonable distance", "적당한 거리"
- **"Flexible" (10km)**: "flexible", "don't mind traveling", "괜찮아요", "10km"
- **"Willing to Travel" (15km)**: "willing to travel", "can go far", "멀어도 괜찮아", "15km"
- **"Anywhere in Seoul" (25km)**: "anywhere", "doesn't matter", "any district", "어디든", "서울 전역", "25km"

**Travel Label Rules:**
- Pick ONE label that best matches willingness to travel
- Default to "Moderate" if unclear or not specified
- Look for explicit distance or travel willingness indicators
- User can change travel distance in any message

---

## 4. SOFT KEYWORDS (LOWEST PRIORITY - SUBJECTIVE PREFERENCES)

Soft keywords are **SUBJECTIVE** qualities used for semantic ranking within filtered results.

### 4A. POSITIVE SOFT KEYWORDS (Desired Qualities)

**Personal Qualities:**
- "friendly", "kind", "patient", "gentle"
- "친절한", "상냥한", "인내심 있는", "부드러운"
- "professional", "전문적인"

**Experience/Expertise:**
- "experienced", "skilled", "expert", "specialist"
- "숙련된", "경험 많은", "전문가"

**Atmosphere:**
- "clean", "modern", "comfortable", "quiet", "spacious"
- "깨끗한", "현대적인", "편안한", "조용한", "넓은"

**Service Quality:**
- "thorough", "detailed", "careful", "attentive"
- "세심한", "자세한", "꼼꼼한"

**Communication:**
- "explains well", "good listener", "clear"
- "설명 잘하는", "소통 잘하는"

**Reputation:**
- "trustworthy", "reliable", "recommended", "popular"
- "믿을만한", "신뢰할 수 있는", "유명한"
...
**Cost:**
- "affordable", "reasonable price", "good value"
- "저렴한", "합리적인", "가성비"

**Characteristics:**
- ✗ Subjective and opinion-based
- ✗ Usually adjectives (descriptive)
- ✗ Require semantic understanding
- ✗ Used for RANKING (not filtering)

### 4B. NEGATIVE SOFT KEYWORDS (Qualities to Avoid)

**Detection Patterns:**
- "avoid X", "not X", "don't want X", "skip X"
- "X 피하고", "X 원하지 않는", "X 싫은"

**Common Negative Qualities:**
- "unfriendly", "rude", "impersonal", "rushed"
- "불친절한", "무뚝뚝한", "성급한"
- "crowded", "busy", "long wait"
- "붐비는", "대기 시간 긴"
- "dirty", "old", "uncomfortable"
- "더러운", "오래된", "불편한"
- "expensive", "overpriced"
- "비싼", "가격이 높은"

**Examples:**
- "friendly, **avoid rude staff**" → soft: ["friendly"], negative: ["rude"]
- "**not crowded** clinics" → negative: ["crowded"]
- "clean, **skip dirty facilities**" → soft: ["clean"], negative: ["dirty"]
- "**avoid long wait times**" → negative: ["long wait times"]

**Characteristics:**
- ✗ Subjective qualities to AVOID
- ✗ Opinion-based
- ✗ Used for DEPRIORITIZING in ranking

---

## 5. ADDITIONAL EXTRACTION RULES

### 5A. LANGUAGE DETECTION
- If 50%+ Korean characters (한글) → "Korean"
- If 50%+ English/ASCII → "English Preferred"

### 5B. GPS COORDINATES (Optional)
...
### 5C. MULTIPLE SIMULTANEOUS CHANGES
**User can update MULTIPLE parameters in ONE message:**
- "내과 말고 치과, 용산구, 주차 필요" → Changes specialty + location + adds hard keyword
- "change to Gangnam, need parking now" → Changes location + adds hard keyword
- "flexible distance, friendly doctor, no weekend hours" → Changes travel + adds soft + adds negative_hard
- "actually closer is better, and I need English support" → Changes travel + adds hard keyword

**Always extract ALL relevant parameters from each message**

### 5D. EXTRACTION PRIORITIES
1. Extract hard keywords FIRST (both positive and negative)
2. Then extract specialty
3. Then extract location and travel distance
4. Finally extract soft keywords (both positive and negative)
5. Maximum 5 keywords per type (prioritize most important)
6. Remove duplicates and synonyms
...
## EXAMPLES (Showing Priority Order):

**Example 1: Hard Keywords Drive the Search**
Input: "I need parking and weekend hours, preferably friendly"
Output: {{
  "specialty": null,
  ...
  "hard_keywords": ["parking", "weekend hours"],
  "soft_keywords": ["friendly"],
  "negative_hard_keywords": [],
  "negative_keywords": []
}}
Reason: Hard keywords extracted FIRST (parking, weekend hours are must-haves), then soft keyword (friendly)

**Example 2: Multiple Changes at Once**
Input: "actually I need 치과 not 내과, and Yongsan area, with parking"
Output: {{
  "specialty": "치과",
...
Reason: Specialty corrected, location added, hard keyword added - all in ONE message

**Example 3: Priority: Hard > Specialty > Location > Soft**
Input: "English-speaking dermatologist with parking in Gangnam, preferably clean and modern"
Output: {{
  "specialty": "피부과",
  "specialty_confidence": 1.0,
  "location": "강남",
  ...
  "travel_label": "Moderate",
  "language_pref": "English Preferred",
  "hard_keywords": ["English-speaking", "parking"],
  "soft_keywords": ["clean", "modern"],
  "negative_hard_keywords": [],
  "negative_keywords": []
}}
Reason: Hard keywords (English, parking) → Specialty (dermatology) → Location (Gangnam) → Soft (clean, modern)

**Example 4: Yongsan as Default Center**
Input: "nearby dentist with parking"
Output: {{
  "specialty": "치과",
  "specialty_confidence": 1.0,
  "location": null,
...
Reason: No location specified → will default to Yongsan-gu area, "nearby" = 1km travel label

**Example 5: Negative Hard Keywords Priority**
Input: "colonoscopy clinic, no weekend hours, must have parking"
Output: {{
  "specialty": "내과",
  ...
  "travel_label": "Moderate",
  "language_pref": "English Preferred",
  "hard_keywords": ["parking", "colonoscopy"],
  "soft_keywords": [],
  "negative_hard_keywords": ["weekend hours"],
  "negative_keywords": []
}}
Reason: Hard requirements first (parking, colonoscopy), negative hard exclusion (no weekend hours), then specialty

**Example 6: Complex Multi-Parameter Update**
Input: "change to Gangnam, flexible distance, need English support and parking, avoid crowded places"
Output: {{
  "specialty": null,
  "specialty_confidence": 0.0,
  "location": "강남",
  ...
  "language_pref": "English Preferred",
  "hard_keywords": ["English support", "parking"],
  "soft_keywords": [],
  "negative_hard_keywords": [],
  "negative_keywords": ["crowded"]
}}
Reason: Multiple changes: location (Gangnam), travel (Flexible), hard keywords (English, parking), negative soft (crowded)

**Example 7: Soft Keywords Are Lowest Priority**
Input: "friendly and clean dentist"
Output: {{
  "specialty": "치과",
  "specialty_confidence": 1.0,
  "location": null,
  ...
  "language_pref": "English Preferred",
  "hard_keywords": [],
  "soft_keywords": ["friendly", "clean"],
  "negative_hard_keywords": [],
  "negative_keywords": []
}}
Reason: Only soft keywords (subjective preferences) - used for ranking, not filtering

**Example 8: Hard Negative Excludes Options**
Input: "내과 in 용산, parking required, no colonoscopy"
Output: {{
  "specialty": "내과",
  "specialty_confidence": 1.0,
  "location": "용산",
  ...
  "hard_keywords": ["parking"],
  "soft_keywords": [],
  "negative_hard_keywords": ["colonoscopy"],
  "negative_keywords": []
}}
Reason: Hard positive (parking) and hard negative (no colonoscopy) extracted first, then specialty and location

**Example 9: Travel Distance Update**
Input: "actually anywhere in Seoul is fine, need weekend hours"
Output: {{
  "specialty": null,
  "specialty_confidence": 0.0,
  "location": null,
  ...
  "language_pref": "English Preferred",
  "hard_keywords": ["weekend hours"],
  "soft_keywords": [],
  "negative_hard_keywords": [],
  "negative_keywords": []
}}
Reason: Travel distance updated to city-wide, hard keyword (weekend hours) extracted

**Example 10: Full Priority Hierarchy**
Input: "insurance and parking required, 피부과, Gangnam, walking distance, trustworthy doctor, avoid rushed service"
Output: {{
  "specialty": "피부과",
  "specialty_confidence": 1.0,
  "location": "강남",
  ...
  "language_pref": "Korean",
  "hard_keywords": ["insurance", "parking"],
  "soft_keywords": ["trustworthy"],
  "negative_hard_keywords": [],
  "negative_keywords": ["rushed"]
}}
Reason: Priority order: Hard (insurance, parking) → Specialty (dermatology) → Location (Gangnam) + Distance (walking) → Soft positive (trustworthy) and negative (rushed)

**Example 11: Single-Word Location (Yongsan)**
Input: "Yongsan"
Output: {{
  "specialty": null,
  "specialty_confidence": 0.0,
...
Reason: User specifying Yongsan area (could be answering location question)

**Example 12: Correction with Multiple Updates**
Input: "no i need internal medicine with colonoscopy, change to flexible distance"
Output: {{
  "specialty": "내과",
  "specialty_confidence": 0.9,
  "location": null,
...
Reason: "no" is correction (ignored), specialty updated, procedure added as hard keyword, travel distance changed
"""
...
# ==========================================
# EXTRACTION_PROMPT_V1 FULL PROMPT
